...
def prepend_model(multi_language_field: Optional[Dict[str, str]]) -> Optional[Dict[str, str]]:
    if multi_language_field is None:
        return None

    return {field_language: f'{prepend.get(field_language)} - {field_value}'
            for field_language, field_value in multi_language_field.items()
            if field_language in prepend}


def map_model_from_dict(raw: RawModel, data_service: DataService) -> Optional[InformationModel]:
    ...
    model.conformsTo = [resource.uri for resource in data_service.conformsTo] if data_service.conformsTo else None
